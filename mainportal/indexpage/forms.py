...
class ProceduresForm(forms.Form):
    ...
    date_start = forms.DateField(label='Дата старта', widget=forms.SelectDateWidget(years=range(2020,2031)), required=False)
    date_end = forms.DateField(label='Дата рассмотрения', widget=forms.SelectDateWidget(years=range(2020,2031)), required=False)
    date_proc = forms.DateField(label='Прием заявок до', widget=forms.SelectDateWidget(years=range(2020,2031)), required=False)
